...
function VmSection() {
  ...
  const save = async () => {
    try {
      await Promise.all([
        dbSetProfile("vmName",    profile.vmName),
        dbSetProfile("vmIp",      profile.vmIp),
        dbSetProfile("vmSshUser", profile.vmSshUser),
        dbSetProfile("vmSshKey",  profile.vmSshKey),
        dbSetProfile("vmSshPass", profile.vmSshPass),
      ]);
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } catch { /* dev mode */ }
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      <SectionHeader title="ATTACK VM — VIRTUALBOX + SSH" icon="⌁" />
      <div style={{ fontSize: 9, color: "rgba(0,212,255,0.35)", marginBottom: 14, lineHeight: 1.7 }}>
        Configure your Linux attack VM (Kali, Parrot, etc.) for Phase 8 offensive operations.
        VM must have SSH enabled and a host-only network adapter.
      </div>

      <Field label="VM NAME (VirtualBox)" value={profile.vmName}
        onChange={v => setProfile({ vmName: v })} placeholder="Kali-Linux" />
      <Field label="VM IP (host-only adapter)" value={profile.vmIp}
        onChange={v => setProfile({ vmIp: v })} placeholder="192.168.56.101" />
      <Field label="SSH USER" value={profile.vmSshUser}
        onChange={v => setProfile({ vmSshUser: v })} placeholder="kali" />
      <Field label="SSH PASSWORD (if no key)" value={profile.vmSshPass}
        onChange={v => setProfile({ vmSshPass: v })} placeholder="kali" type="password" />
      <Field label="SSH KEY PATH — optional (leave blank if using password)" value={profile.vmSshKey}
        onChange={v => setProfile({ vmSshKey: v })} placeholder="C:\Users\abdul\.ssh\t_vm_key" />

      <div style={{
        background: "rgba(0,212,255,0.02)", border: "1px solid rgba(0,212,255,0.08)",
        borderRadius: 4, padding: "10px 14px", marginBottom: 14, fontSize: 9,
        color: "rgba(0,212,255,0.45)", lineHeight: 1.8,
      }}>
        <div style={{ color: "#00d4ff", marginBottom: 4, letterSpacing: 2, fontSize: 8 }}>ONE-TIME SETUP</div>
        1. VirtualBox → VM Settings → Network → Adapter 1 → Host-Only Adapter<br />
        2. On VM: <code style={{ color: "#a0f4ff" }}>sudo apt install openssh-server && sudo systemctl enable --now ssh</code><br />
        3. On VM: <code style={{ color: "#a0f4ff" }}>ip a</code> → note the 192.168.56.x IP<br />
        4. On Windows: <code style={{ color: "#a0f4ff" }}>ssh-keygen -t ed25519 -f C:\Users\abdul\.ssh\t_vm_key</code><br />
        5. Copy key: <code style={{ color: "#a0f4ff" }}>type t_vm_key.pub | ssh user@VM_IP "cat &gt;&gt; ~/.ssh/authorized_keys"</code>
      </div>
      ...
      <button onClick={save} style={{
        ...
        border: `1px solid ${saved ? "rgba(0,255,136,0.4)" : "rgba(0,212,255,0.3)"}`,
        color: saved ? "#00ff88" : "#00d4ff",
        borderRadius: 3, cursor: "pointer", fontFamily: "inherit", transition: "all 0.3s",
      }}>
        {saved ? "SAVED ✓" : "SAVE VM CONFIG"}
      </button>
    </div>
  );
}
...
function PersistenceSection() {
  const [status, setStatus] = useState<"unknown"|"running"|"stopped"|"not_installed">("unknown");
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      const result = await invoke<string>("check_service_status");
  ...
  const statusColor = {
    running:       "#00ff88",
    stopped:       "#ff6600",
    not_installed: "rgba(0,212,255,0.3)",
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      <SectionHeader title="PERSISTENCE — AUTO-START" icon="⚙" />
      <div style={{ fontSize: 9, color: "rgba(0,212,255,0.35)", marginBottom: 14, lineHeight: 1.7 }}>
        Run T brain as a Windows service — starts on boot, restarts if it crashes.
        Tray icon appears at login with quick controls.
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 9, color: statusColor, letterSpacing: 2 }}>
          {statusLabel}
        </span>
        <button onClick={checkStatus} disabled={checking} style={{
        ...
        }}>{checking ? "CHECKING..." : "CHECK STATUS"}</button>
      </div>

      {/* Install instructions */}
      <div style={{
        background: "rgba(0,212,255,0.02)", border: "1px solid rgba(0,212,255,0.08)",
        borderRadius: 4, padding: "12px 14px", fontSize: 9,
        color: "rgba(0,212,255,0.45)", lineHeight: 1.9, marginBottom: 14,
      }}>
        <div style={{ color: "#00d4ff", marginBottom: 6, letterSpacing: 2, fontSize: 8 }}>
          ONE-TIME SETUP (run as Administrator)
        </div>
        <div>
          <span style={{ color: "rgba(0,212,255,0.3)" }}>1. Install deps: </span>
          <code style={{ color: "#a0f4ff" }}>pip install pywin32 pystray pillow</code>
        </div>
        <div>
          <span style={{ color: "rgba(0,212,255,0.3)" }}>2. Install service: </span>
          <code style={{ color: "#a0f4ff" }}>cd T\brain && python service/install.py</code>
        </div>
        <div>
          <span style={{ color: "rgba(0,212,255,0.3)" }}>3. Uninstall: </span>
          <code style={{ color: "#a0f4ff" }}>python service/install.py --uninstall</code>
        </div>
      </div>

      {/* What it does */}
      <div style={{
        background: "rgba(0,212,255,0.01)", border: "1px solid rgba(0,212,255,0.06)",
        borderRadius: 4, padding: "10px 14px", fontSize: 9,
        color: "rgba(0,212,255,0.4)", lineHeight: 1.8,
      }}>
        {[
          ["Windows Service",   "Brain starts on boot — before you log in"],
          ["Auto-restart",      "Restarts within 5s if it crashes (up to 20 times/hour)"],
          ["Tray icon",         "Status indicator + open/restart/stop from system tray"],
          ["Log rotation",      "Brain logs auto-rotated at 5MB, 5 archives kept"],
          ["Log location",      "%APPDATA%\\T\\logs\\"],
        ].map(([label, desc]) => (
          <div key={label} style={{ display: "flex", gap: 12, marginBottom: 3 }}>
            <span style={{ color: "#00d4ff", minWidth: 130 }}>{label}</span>
            <span>{desc}</span>
          </div>
        ))}
      </div>

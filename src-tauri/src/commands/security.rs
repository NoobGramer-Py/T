...
pub struct InjectionResult {
    ...
    pub verdict:           String,    // "clean" | "suspicious" | "injected"
    pub suspicious_dlls:   Vec<String>,
    pub private_exec_regions: u32,
    pub details:           Vec<String>,
}

#[tauri::command]
pub async fn detect_process_injection(pid: u32) -> Result<InjectionResult, String> {
...
$suspicious = $modules | Where-Object {{
    $f = $_.ToLower()
    $f -and
    -not $f.StartsWith($sysRoot) -and
    -not $f.StartsWith('c:\program files') -and
    -not $f.StartsWith('c:\program files (x86)')
}}
$name = $proc.Name
Write-Output "NAME:$name"
$suspicious | ForEach-Object {{ Write-Output "DLL:$_" }}
Write-Output "DONE"
"#,
                    pid = pid
                ),
        ...
        let mut suspicious_dlls: Vec<String> = Vec::new();

        for line in text.lines() {
            if let Some(name) = line.strip_prefix("NAME:") {
                process_name = name.trim().to_string();
            } else if let Some(dll) = line.strip_prefix("DLL:") {
                suspicious_dlls.push(dll.trim().to_string());
            }
        }

        let verdict = if suspicious_dlls.len() >= 3 {
            "injected"
        } else if !suspicious_dlls.is_empty() {
            "suspicious"
        } else {
            "clean"
        };
        ...
        if !suspicious_dlls.is_empty() {
            details.push(format!(
                "{} DLL(s) loaded from non-standard paths",
                suspicious_dlls.len()
            ));
        } else {
            details.push("All loaded modules are from standard system paths".to_string());
        }
        ...
        Ok(InjectionResult {
            ...
            suspicious_dlls,
            private_exec_regions: 0,
            details,
        })
    }

    #[cfg(not(target_os = "windows"))]
    Err("Process injection detection is only available on Windows".to_string())
}
...
pub async fn get_arp_table() -> Result<Vec<ArpEntry>, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        for line in text.lines() {
            let line = line.trim();
            // Interface header: "Interface: 192.168.1.x --- 0x..."
            if line.starts_with("Interface:") {
                let parts: Vec<&str> = line.split_whitespace().collect();
                current_iface = parts.get(1).unwrap_or(&"").to_string();
                continue;
            }
            // ARP entry line: "  192.168.1.1    aa-bb-cc-dd-ee-ff    dynamic"
            let parts: Vec<&str> = line.split_whitespace().collect();
            if parts.len() >= 3 {
                let ip  = parts[0];
                let mac = parts[1];
                let typ = parts[2];
                // Skip header row
                if ip == "Internet" || mac == "Physical" { continue; }
                // Normalise MAC: replace '-' with ':'
                let mac_norm = mac.replace('-', ":");
    ...
    #[cfg(not(target_os = "windows"))]
    {
        // Linux / macOS fallback using 'arp -n'
        let output = Command::new("arp")
            .arg("-n")
            .output()
            .map_err(|e| e.to_string())?;
...
pub struct UsbDevice {
    ...
    pub device_type:  String,   // "rubber_ducky" | "omg_cable" | "android" | "ios" | "unknown"
    pub drive_letter: String,   // populated if mass storage
}

#[tauri::command]
pub async fn detect_usb_devices() -> Result<Vec<UsbDevice>, String> {
    // Known VID:PID pairs for attack hardware
    let known_hid: &[(&str, &str, &str)] = &[
        ("03EB", "2401", "rubber_ducky"),   // Hak5 Rubber Ducky
        ("0FC0", "0001", "omg_cable"),      // O.MG Cable
        ("1209", "0001", "omg_cable"),      // O.MG Cable alt VID
    ];
    let known_android: &[&str] = &[
        "18D1",  // Google (Pixel, Nexus)
        "04E8",  // Samsung
        "2717",  // Xiaomi
        "1BBB",  // Motorola
        "0BB4",  // HTC
        "04DA",  // Panasonic/Kyocera
        "0FCE",  // Sony
        "22D9",  // OPPO
        "2A70",  // OnePlus
        "1D4D",  // Pegatron
    ];
    let known_ios: &[&str] = &["05AC"];  // Apple
...
Where-Object { $_.Class -in @('USB','HIDClass','AndroidUsbDeviceClass','Ports','Image','WPD') } |
Select-Object DeviceID, FriendlyName |
...
"#,
    ...
    for item in &items {
        ...
        let description = item["FriendlyName"].as_str().unwrap_or("Unknown").to_string();

        // Extract VID and PID from DeviceID string e.g. USB\VID_18D1&PID_4EE7\...
        let vid = device_id.split("VID_").nth(1)
            .and_then(|s| s.split('&').next())
            .unwrap_or("")
            .to_uppercase();
        let pid = device_id.split("PID_").nth(1)
            .and_then(|s| s.split('\\').next())
            .unwrap_or("")
            .to_uppercase();

        let device_type = if known_hid.iter().any(|(v, p, _)| *v == vid && *p == pid) {
            known_hid.iter()
                .find(|(v, p, _)| *v == vid && *p == pid)
                .map(|(_, _, t)| t.to_string())
                .unwrap_or_else(|| "hid".to_string())
        } else if known_android.contains(&vid.as_str()) {
            "android".to_string()
        } else if known_ios.contains(&vid.as_str()) {
            "ios".to_string()
        } else if device_id.contains("USB") && !vid.is_empty() {
            "unknown".to_string()
        } else {
            continue;
        };
    ...
    // Enrich with drive letters for mass storage devices
    let drives_out = Command::new("wmic")
        .args(["logicaldisk", "get", "DeviceID,VolumeName"])
        .output()
        .unwrap_or_else(|_| std::process::Output {
            status: std::process::ExitStatus::default(),
            stdout: vec![], stderr: vec![],
        });
    let drives_text = String::from_utf8_lossy(&drives_out.stdout);
    for dev in &mut devices {
        for line in drives_text.lines() {
            let parts: Vec<&str> = line.split_whitespace().collect();
            if parts.len() >= 2 {
                let label = parts.get(1).unwrap_or(&"").to_uppercase();
                if label.contains("DUCKY") || label.contains("OMG") {
                    dev.drive_letter = parts[0].to_string();
                }
            }
        }
    }

...
## Phase 8 — Full Spectrum Offensive Security (LOCKED SPEC)

> **Status**: Planning complete. Ready to execute.
> **Author decision — Rust additions**: Process injection detection + ARP table (2 commands).
> Raw packet capture deferred (requires Npcap driver pre-installed on Abdul's machine — VM covers it).
...
| Capability | Location |
|---|---|
| nmap scan, port scan, IP rep, CVE, full port scan | `security.rs` + SecurityPanel |
| Local credential dump (LSASS/SAM/browsers/WiFi) | `brain/local_access/` + SecurityPanel LOCAL ACCESS tab |
| Process memory inspector | `brain/local_access/memory_inspector.py` |
| Confirmation flow architecture | `brain/agents/executor.py`, `brain/hardware/command_router.py` |
| Security event log table | SQLite `security_log` |
| Proactive security alerts | `brain/proactive/` |
| OSINT: IP intel, email OSINT, HIBP | SecurityPanel existing tabs |
...
```
SecurityPanel.tsx  (9 new tabs)
        │
        │ WebSocket (existing bridge)
        ▼
brain/core/engine.py
        │ offensive_action / offensive_confirm / vm_command
        ▼
brain/offensive/
├── orchestrator.py    ← confirmation gate, tool dispatch, action logging
├── vm_bridge.py       ← VBoxManage lifecycle + paramiko SSH + output streaming
├── stream.py          ← subprocess stdout → WebSocket chunks in real-time
├── tool_check.py      ← verify tool on VM, offer install with confirmation
└── tool_catalog.py    ← registry of 60+ tools: name, install_cmd, category, description

src-tauri/src/commands/security.rs  ← 2 new Rust commands added to existing file
```

---

### VM Setup (One-Time — Abdul Does This)

T will guide Abdul through this on first VM panel open:

```
1. VirtualBox → Settings → Network → Adapter 1 → Host-Only Adapter
2. On VM: sudo apt install openssh-server && sudo systemctl enable --now ssh
3. On VM: ip a  →  note the IP (usually 192.168.56.x)
4. On Windows host: ssh-keygen -t ed25519 -f C:\Users\abdul\.ssh\t_vm_key
5. On Windows host: type C:\Users\abdul\.ssh\t_vm_key.pub | ssh user@VM_IP "cat >> ~/.ssh/authorized_keys"
6. In T Settings → VM section: enter VM name, VM IP, SSH user, key path
```

T stores these in the existing `profile` table (4 new keys: `vm_name`, `vm_ip`, `vm_ssh_user`, `vm_ssh_key`).

---

### Tool Catalog — 60+ Tools Across 9 Categories

All run on the Linux VM via SSH. T checks for each tool before running, offers to install if missing.

**Reconnaissance**
| Tool | Purpose | Install |
|---|---|---|
| nmap | Port/service/OS scan (host-side Rust already covers basic) | `apt install nmap` |
| masscan | Ultra-fast port scanner | `apt install masscan` |
...
| theHarvester | Email, subdomain, IP from public sources | `apt install theharvester` |
| recon-ng | Modular OSINT framework | `apt install recon-ng` |
| whatweb | Web fingerprinting | `apt install whatweb` |
| shodan | Shodan CLI search | `pip3 install shodan` |
| maltego | Link analysis (GUI — T opens on VM) | `apt install maltego` |
| dnsx | DNS resolution at scale | `go install github.com/projectdiscovery/dnsx/cmd/dnsx@latest` |

**Web Application**
| Tool | Purpose | Install |
|---|---|---|
| nikto | Web server vulnerability scan | `apt install nikto` |
| sqlmap | SQL injection automation | `apt install sqlmap` |
| ffuf | Web fuzzer (dirs, params, vhosts) | `apt install ffuf` |
| wfuzz | Advanced web fuzzer | `apt install wfuzz` |
| gobuster | Directory/DNS/vhost brute force | `apt install gobuster` |
| wpscan | WordPress vulnerability scanner | `gem install wpscan` |
| xsstrike | XSS detection and exploitation | `pip3 install xsstrike` |
| commix | Command injection tester | `apt install commix` |
...
| hydra | Online brute force (SSH, FTP, HTTP, etc.) | `apt install hydra` |
| medusa | Parallel online brute force | `apt install medusa` |
| crunch | Wordlist generator | `apt install crunch` |
| cewl | Custom wordlist from website | `apt install cewl` |
| cupp | Common User Passwords Profiler | `apt install cupp` |
| rsmangler | Wordlist mangler | `apt install rsmangler` |

**Exploitation**
| Tool | Purpose | Install |
|---|---|---|
| metasploit | Full exploitation framework | `apt install metasploit-framework` |
| searchsploit | Local exploit-db search | `apt install exploitdb` |
| BeEF | Browser exploitation framework | `apt install beef-xss` |
| routersploit | Router/embedded device exploits | `pip3 install routersploit` |

**WiFi**
| Tool | Purpose | Install |
|---|---|---|
| aircrack-ng | WEP/WPA cracking suite | `apt install aircrack-ng` |
| airodump-ng | Wireless packet capture | (part of aircrack-ng suite) |
| aireplay-ng | Packet injection/deauth | (part of aircrack-ng suite) |
| airmon-ng | Monitor mode management | (part of aircrack-ng suite) |
| hcxdumptool | Advanced PMKID/handshake capture | `apt install hcxdumptool` |
| hcxtools | Convert captures for hashcat | `apt install hcxtools` |
| wifite2 | Automated WiFi auditor | `apt install wifite` |
| kismet | Wireless network detector/sniffer | `apt install kismet` |
| bettercap | WiFi/BLE/network swiss-army-knife | `apt install bettercap` |

**MITM & Network**
| Tool | Purpose | Install |
|---|---|---|
| ettercap | MITM ARP poisoning + sniffing | `apt install ettercap-graphical` |
| arpspoof | ARP cache poisoning | `apt install dsniff` |
| mitmproxy | Interactive HTTPS proxy | `apt install mitmproxy` |
| sslstrip | SSL stripping | `pip3 install sslstrip` |
| responder | LLMNR/NBT-NS poisoner + hash capture | `apt install responder` |
| tcpdump | Packet capture (CLI) | `apt install tcpdump` |
| tshark | Packet capture (Wireshark CLI) | `apt install tshark` |
| tcpflow | TCP stream reassembly | `apt install tcpflow` |
| ngrep | Network grep | `apt install ngrep` |

**Payload & Evasion**
| Tool | Purpose | Install |
|---|---|---|
| msfvenom | Payload generator (part of Metasploit) | (with metasploit) |
| veil | AV-evading payload generator | `apt install veil` |
| shellter | PE injection shellcode framework | `apt install shellter` |
| unicorn | Magic Unicorn shellcode injection | `git clone https://github.com/trustedsec/unicorn` |
| upx | Binary packer/compressor | `apt install upx` |

**OSINT & Location**
| Tool | Purpose | Install |
|---|---|---|
| phoneinfoga | Phone number OSINT | `pip3 install phoneinfoga` |
| sherlock | Username search across 300+ sites | `pip3 install sherlock-project` |
| holehe | Email → account check (70+ sites) | `pip3 install holehe` |
| maigret | Username OSINT (3000+ sites) | `pip3 install maigret` |
| exiftool | Extract GPS/metadata from files | `apt install libimage-exiftool-perl` |
| creepy | Geolocation from social media | `apt install creepy` |
| OSRFramework | Username/email/DNS OSINT | `pip3 install osrframework` |
| GHunt | Google account OSINT | `pip3 install ghunt` |

**Forensics & Steganography**
| Tool | Purpose | Install |
|---|---|---|
| binwalk | Firmware/binary analysis | `apt install binwalk` |
| steghide | Steganography detect/extract | `apt install steghide` |
| stegsolve | Image steg analysis (GUI) | (Java jar — T opens on VM) |
| volatility3 | Memory forensics | `pip3 install volatility3` |
| foremost | File carving | `apt install foremost` |
| autopsy | Digital forensics GUI | `apt install autopsy` |
| strings | Extract strings from binary | (system tool) |
...
```json
// Tauri → Brain
{"type": "offensive_action", "id": "uuid", "tool": "hashcat", "params": {"hash": "...", "wordlist": "rockyou.txt", "mode": "1000"}}
{"type": "offensive_confirm",  "id": "uuid", "confirmed": true}
{"type": "vm_command",  "action": "start|stop|status|snapshot", "snapshot_name": "clean"}
{"type": "tool_install", "id": "uuid", "tool": "hashcat"}

// Brain → Tauri
{"type": "offensive_confirm_request", "id": "uuid", "tool": "hashcat", "command": "hashcat -m 1000 ...", "description": "Crack NTLM hash using rockyou.txt dictionary"}
{"type": "offensive_stream",  "id": "uuid", "tool": "hashcat", "chunk": "...stdout line..."}
{"type": "offensive_done",    "id": "uuid", "tool": "hashcat", "exit_code": 0, "duration_s": 14.2}
{"type": "offensive_error",   "id": "uuid", "tool": "hashcat", "error": "hashcat not found. Install it?"}
{"type": "vm_status",  "running": true, "ip": "192.168.56.101", "ssh_ok": true, "tools_checked": {"hashcat": true, "metasploit": false}}
{"type": "tool_missing", "id": "uuid", "tool": "hashcat", "install_cmd": "sudo apt install hashcat -y"}
```
...
**Python — brain:**
```
brain/offensive/__init__.py
brain/offensive/orchestrator.py      ← confirmation gate, dispatch, action logging to security_log
brain/offensive/vm_bridge.py         ← VBoxManage (start/stop/snapshot) + paramiko SSH + stream
brain/offensive/stream.py            ← async subprocess stdout → WebSocket chunks
brain/offensive/tool_check.py        ← ssh + which <tool>, return bool + install_cmd
brain/offensive/tool_catalog.py      ← dict of 60+ tools: {name, category, install_cmd, description, dangerous}
```

**Python — engine wiring:**
```
brain/core/engine.py         ← offensive_action / offensive_confirm / vm_command / tool_install handlers
brain/requirements.txt       ← + paramiko>=3.4.0
```

**Rust — 2 new commands in existing security.rs:**
```
detect_process_injection(pid: u32) → injected_dlls, suspicious_regions, verdict
get_arp_table()                    → list of {ip, mac, interface, type}
```

**Frontend:**
```
src/components/security/SecurityPanel.tsx  ← 9 new tabs (see tab map below)
src/hooks/useBridge.ts                      ← useOffensive() hook
src/store/index.ts                          ← vm_name, vm_ip, vm_ssh_user, vm_ssh_key in UserProfile
src/components/settings/SettingsPanel.tsx   ← VM configuration section
src/lib/tauri.ts                            ← detectProcessInjection(), getArpTable()
```

---

### New SecurityPanel Tab Map

Existing tabs (15): scanner, iprep, ports, processes, dns, vpn, firewall, password, url, log, ipintel, emailosint, cve, fullscan, localaccess

New tabs (9):
| Tab ID | Label | Primary Tools |
|---|---|---|
| `vm` | VM | VBoxManage status, start/stop, SSH terminal, tool installer |
| `crack` | CRACK | hashcat (NTLM/MD5/SHA/WPA), john, mode selector, wordlist picker |
| `exploit` | EXPLOIT | Metasploit module search, msfconsole stream, searchsploit |
| `wifi` | WIFI | airmon-ng, airodump, aireplay deauth, hcxdumptool, wifite2, WPA crack |
| `mitm` | MITM | arpspoof/ettercap setup, mitmproxy, responder, tcpdump stream |
| `payload` | PAYLOAD | msfvenom builder (OS/arch/format), veil, shellter, output download |
| `web` | WEB | nikto, sqlmap, ffuf/gobuster, wpscan, nuclei — target + options form |
| `osint` | OSINT | phoneinfoga, sherlock, holehe, maigret, creepy, GHunt, exiftool |
| `recon` | RECON | masscan, amass, subfinder, theHarvester, recon-ng, whatweb |

---

### Rust Command Decisions

**`detect_process_injection(pid: u32)`** — YES, build
- Opens process handle with `PROCESS_QUERY_INFORMATION | PROCESS_VM_READ`
- Enumerates loaded modules via `EnumProcessModules`
- Scans for: unsigned DLLs not in System32/SysWOW64, executable private memory regions, mismatched PE headers
- Returns: list of suspicious DLLs + verdict (clean / suspicious / injected)
- Value: complements local_access memory inspector, useful for defensive auditing

**`get_arp_table()`** — YES, build
- Calls `GetIpNetTable2` Windows API
- Returns full ARP cache: IP, MAC, interface, entry type (static/dynamic/invalid)
- Value: baseline for MITM detection, shows ARP poisoning in progress

**Raw packet capture** — DEFERRED
- Requires Npcap driver on Windows, can't assume it's installed
- VM's tcpdump/tshark covers this completely
- Can add in Phase 9 if needed

---

### Confirmation Model (Non-Negotiable)

Every offensive action:
```
1. T shows: tool name + exact command + plain-English description + risk level
2. T sends: offensive_confirm_request to SecurityPanel
3. User clicks CONFIRM in the active tab — or types YES in chat
4. Action executes with stdout streaming to the tab's output area
5. T logs: timestamp, tool, target, full command, exit_code → security_log table
6. T never chains — next action requires a new confirmation
```

Risk levels shown in the confirmation modal:
- `LOW` — read-only, no traffic generated (searchsploit, exiftool, sherlock)
- `MEDIUM` — active scanning, network traffic to target (nmap, nikto, sqlmap)
- `HIGH` — intrusive, may trigger IDS/alerts (hydra, sqlmap --dbs, metasploit exploit)
- `CRITICAL` — illegal without authorization (deauth, ARP poison on networks you don't own)

T always appends for CRITICAL: *"Only run this on networks and systems you own or have explicit written permission to test."*

---

### Action Logging Schema (security_log table — already exists, extend)

```sql
-- Add columns to existing security_log:
ALTER TABLE security_log ADD COLUMN tool      TEXT;
ALTER TABLE security_log ADD COLUMN target    TEXT;
ALTER TABLE security_log ADD COLUMN command   TEXT;
ALTER TABLE security_log ADD COLUMN exit_code INTEGER;
ALTER TABLE security_log ADD COLUMN duration_s REAL;
```

---

### Phase 8 Hard Rules (Locked)

1. Every offensive action → single YES confirmation → no batching, no chains
2. T describes action in plain English + shows exact command before asking
3. Every action logged to `security_log` with full command + exit code
4. VM tool install requires separate confirmation from running the tool
5. T never triggers offensive actions from proactive engine — hard block
6. CRITICAL risk tools display legal warning every single time, no exceptions
7. VM SSH key path never leaves the settings panel — never sent in WebSocket messages
8. Stream output is sanitized — ANSI escape codes stripped before sending to Tauri
...
```
# brain/requirements.txt
paramiko>=3.4.0          ← SSH client for VM communication
```

No new Rust crates needed — Windows process/ARP APIs available through `winapi` crate, but we'll use the `windows` crate pattern already established in the existing `security.rs` using `std::process::Command` for feasibility. If direct API calls are needed, `winapi = { version = "0.3", features = [...] }` is added.

---

### Execution Order (When Abdul Gives the Go)

```
Step 1 — Brain offensive module
  brain/offensive/__init__.py
  brain/offensive/tool_catalog.py      ← all 60+ tools registered here
  brain/offensive/vm_bridge.py         ← VBoxManage + paramiko
  brain/offensive/stream.py            ← async stdout streaming
  brain/offensive/tool_check.py        ← SSH which + install offer
  brain/offensive/orchestrator.py      ← confirmation gate + dispatch + logging

Step 2 — Engine wiring
  brain/core/engine.py                 ← 4 new message handlers
  brain/requirements.txt               ← + paramiko

Step 3 — Rust additions
  src-tauri/src/commands/security.rs   ← detect_process_injection, get_arp_table
  src/lib/tauri.ts                     ← 2 new TS bindings

Step 4 — Frontend
  src/store/index.ts                   ← VM fields in UserProfile
  src/components/settings/SettingsPanel.tsx  ← VM config section
  src/hooks/useBridge.ts               ← useOffensive() + useVmStatus() hooks
  src/components/security/SecurityPanel.tsx  ← 9 new tabs

Step 5 — Integration test + commit
```

**Done when**: Abdul opens VM tab → T checks VM status → shows which of 60+ tools are installed → Abdul selects `CRACK` tab → pastes an NTLM hash → T shows "hashcat -m 1000 hash.txt rockyou.txt — will attempt to crack NTLM hash using dictionary attack" → Abdul clicks CONFIRM → output streams live into the panel → result logged.
